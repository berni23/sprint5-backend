...
Both paths share the same core logic and requirements — the only difference is how the user interacts with the app.
...
- Use **Enum** to define the set of valid operations
...
If you finish the core requirements and want to keep going:
...
- **About** — add an `--action=about` flag (Path A) or `/about` endpoint (Path B) that returns information about the app, the author, and what it can do
